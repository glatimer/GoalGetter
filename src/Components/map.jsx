...
export default function MapDisplay() {
      ...
      if (source && destination) {
        try {
          const response = await axios.get(
            `https://maps.googleapis.com/maps/api/directions/json?origin=${source[0]},${source[1]}&destination=${destination[0]},${destination[1]}&key=YOUR_GOOGLE_MAPS_API_KEY`
          );
  ...
  const handleSourceSearch = async () => {
    try {
      const response = await axios.get(
        `https://nominatim.openstreetmap.org/search?format=json&q=${sourceSearchInput}`
      );
  ...
  const handleDestinationSearch = async () => {
    try {
      const response = await axios.get(
        `https://nominatim.openstreetmap.org/search?format=json&q=${destinationSearchInput}`
      );
      if (response.data.length > 0) {
        const { lat, lon } = response.data[0];
          ...
          {source && Array.isArray(source) && source[0] && source[1] && (
            <Marker position={source} icon={greenIcon}>
              <Popup>
                ...
                <div>
                  <strong>Latitude:</strong> {source[0].toFixed(4)},{" "}
                  <strong>Longitude:</strong> {source[1].toFixed(4)}
                </div>
              </Popup>
            </Marker>
          )}
      ...
      <div className="sidebar">
        <div>
          ...
          <div>
            <strong>Source:</strong>{" "}
            {source
              ? `Latitude: ${source[0]?.toFixed(4)}, Longitude: ${source[1]?.toFixed(4)}`
              : "Click on the map to set source"}
          </div>
          <div>
            <strong>Destination:</strong>{" "}
            {destination
              ? `Latitude: ${destination[0]?.toFixed(4)}, Longitude: ${destination[1]?.toFixed(4)}`
          ...
          <div>
            ...
            <div>
              <br></br>
              <strong className="heading-clrs">Estimated Time:</strong>{" "}
